...
export const signUpUser = async (firstName: string, lastName: string, email: string, password: string): Promise<IUser> => {
...
export const loginUser = async (email: string, password: string): Promise<LoginResponse> => {
  ...
  if (!user.isVerified) {
    throw new ErrorHandler(400, 'Account not verified. Please check your email.');
  }
